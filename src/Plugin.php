...
class Plugin {
	...
	public static function Install(GenericEvent $event) {
		...
		$serviceCategory = $plugin->add_service_category('licenses', 'abuse', 'Abuse');
		$plugin->define('SERVICE_TYPES_ABUSE', $serviceCategory);
		$serviceType = $plugin->add_service_type($serviceCategory, 'licenses', 'Abuse');
		$plugin->add_service($serviceCategory, $serviceType, 'licenses', 'Abuse License', 10.00, 0, 1, 1, '');
		$plugin->add_service($serviceCategory, $serviceType, 'licenses', 'Abuse Type2 License', 11.95, 0, 1, 2, '');
